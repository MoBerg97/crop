...
pestindex = pest %>%
  ...
  mutate(conc = paste0(qualifier,value_fin))
...
#Intersection of the cropindex with the pestindex
st_crs(pestindex)
st_crs(ci)
...
tm_shape(nrw) +
  tm_borders() +
tm_shape(pestcrop) +
  tm_fill('totsum_dt', style='pretty', palette='Greens', textNA='district yield below average', 
          title='total yield in dt/ha') +
  tm_symbols(size = 'value_fin', scale=3, col='psm_type' ,palette='Set3', alpha=0.8, border.alpha = 0, 
             title.size ='value of pesticide samples', title.col = 'type of pesticide', title.shape='') +

tm_layout(main.title='Fig.11 Pesticide index and crop index', title.snap.to.legend =TRUE, legend.outside = TRUE, legend.outside.position='right')
```
