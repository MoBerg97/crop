...
cropkreis = fin %>%
  ...
  #mark districts of type 'city' and total crop yield below average of all districts
  ungroup() %>%
  mutate(city = case_when(str_detect(gen, 'Stadt') ~ TRUE,
                          ! str_detect(gen, 'Stadt')~ FALSE)) %>%
  group_by(gn) %>%
  mutate(totsum_dt = sum(sum_dt)) %>%
  ungroup()%>%
  mutate(lowyield = case_when(totsum_dt<mean(totsum_dt) ~ TRUE,
                              totsum_dt>=mean(totsum_dt) ~ FALSE))
...
tm_scale_bar()+
  tm_shape(ci) +
  tm_fill('totsum_dt', style='pretty', palette='Greens', textNA='district yield below average', title='total yield in dt/ha') +
tm_shape(nrw) +
  tm_borders() +
...
tm_scale_bar()+
  tm_shape(ci) +
  tm_fill('totsum_dt', style='pretty', palette='Greens', 
          textNA='district yield below average', title='total yield in dt/ha') +
tm_shape(nrw) +
  tm_borders() +
tm_shape(pest) +
  tm_symbols(size = 'value_fin', scale=3, col='psm_type' ,palette='Set3', alpha=0.8, border.alpha = 0, 
...
#                    TRUE ~ FALSE))
# #####################################   WIP   #################################################
# # count how often certain pesticides were sampled to choose highest ones
# pestdt = as.data.table(pest)
# counts = pestdt[, .(rowCount =.N), by = c('cas','name','psm_type')]
# counts = setorder(counts, rowCount)
# #choose three herbicides with many observations and an insecticide and an fungicide
# # fungicide   66063-05-6	Pencycuron
# # herbicides 	5915-41-3	  Terbuthylazin
# #             34123-59-6	Isoproturon
# #             1912-24-9	  Atrazin
# # insecticide 534-52-1	  4,6-Dinitro-o-Cresol

pestindex = pest %>%
  select(site_id, area_sqkm, atkis_perc, name, cas, qualifier, value) %>%
  mutate(atkis_perc = atkis_perc*100) %>% #agricultural proportion converted into percentage
  # filter(cas %in% c('66063-05-6','5915-41-3','34123-59-6','1912-24-9','534-52-1')) %>%
  mutate(conc = paste0(qualifier,value))
#Intersection of the cropindex with the pestindex IS THIS NECESSARY or is tm_shape() layering better?
st_crs(pestindex)
st_crs(ci)
