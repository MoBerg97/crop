...
```{r overview year}
# average yield of crops per year
mn_cropjahr = fin %>%
  group_by(year, crop) %>%
  summarise(mn_dt = mean(dt.ha))
...
highyield = ggplot(filter(mn_cropjahr,mn_dt>100), aes(x=year,y=mn_dt, group = crop, colour = crop)) + 
...
lowyield = ggplot(filter(mn_cropjahr,mn_dt<100), aes(x=year,y=mn_dt, group=crop)) + 
  ggtitle('Fig.1 Average yield of crops for each year from 2005 to 2015') +
  geom_point() +
  geom_line() + 
...
The graph shows, that some previously largely harvested crops, fell significantly.
Therefore a distinction between the years from 2005 to 2006, from 2007 to 2009 and from 2010 to 2015 was useful.
