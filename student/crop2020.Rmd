...
#TODO create equal breaks
tm_yearspan1 = tm_shape(cropyearspan1) + 
  tm_sf('mn_dt', palette='RdYlBu', style='equal', id='dt/ha', title='dt/ha') + 
tm_compass() + 
tm_scale_bar() +
tm_layout(title='Fig.5 Average of the total crop yield per year for 2005 to 2009', title.bg.color=TRUE)
#map for years 2010 to 2015
tm_yearspan2 = tm_shape(cropyearspan2) + 
  tm_sf('mn_dt', palette='RdYlBu', style='equal', id='dt/ha', title='dt/ha') + 
tm_compass() + 
tm_scale_bar() +
tm_layout(title='Fig.6 Average of the total crop yield per year for 2010 to 2015', title.bg.color=TRUE)
#map plotting
...
abbcode
#####################################   WIP   #################################################
#TODO get tm_facets working maybe with tmap_arrange
# mn_cropkreis = fin %>%
#   filter(year>=2010) %>%
...
cisimple = ci %>%
  filter(totsum_dt>=mean(unique(totsum_dt)))
tm_compass() + tm_scale_bar() + tm_shape(fin) + tm_polygons() + tm_shape(cisimple) + tm_bubbles(size='totsum_dt',  col='totsum_dt', alpha=0.5, scale=3)  + tm_shape(nrw) + tm_borders(lwd = 2, col='grey30')+ tm_layout(title='Fig.9 Crop index', title.bg.color=TRUE)
```
...
#TODO vor Intersection niedrige aktisperc aussortieren
aktischeck = pest %>%
  group_by(site_id) %>%
  summarise(atkis_perc = 100*mean(atkis_perc)) %>%
  arrange(atkis_perc)
summary(aktischeck$atkis_perc)
#TODO intersect with aktischeck
# choose highest atkis_perc per district
# save names of filtered sites
# filter original pest for those filtered sites
st_crs(pest)
st_crs(cisimple)
cisimple = st_transform(cisimple, crs = st_crs(pest))
pestcrop = st_intersection(cisimple, pest)
#####################################   WIP   #################################################
#TODO barplot Pestizide mit höchsten Konzentrationen
ggplot(pest, aes(x=))
#TODO calculate pest index - sample site with highest agricultural proportion
pestindex = pestcrop %>%
