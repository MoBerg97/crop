...
tm_compass() + tm_scale_bar() + tm_shape(fin) + tm_polygons() + tm_shape(cisimple) + tm_bubbles(size='sum_dt',  col='sum_dt', alpha=0.5, scale=3)  + tm_shape(nrw) + tm_borders(lwd = 2, col='grey30')+ tm_layout(title='Fig.9 Crop index', title.bg.color=TRUE)
```
...
st_crs(pest)
st_crs(ci)
ci = st_transform(ci, crs = st_crs(pest))
pestcrop = st_intersection(ci, pest)
#####################################   WIP   #################################################
