...
fin = fin %>%
  ...
  mutate(cropcat2 = plyr::mapvalues(cropcat2, 
            ...
            to= c(NA,NA,NA))) %>%
  na.omit()
cropcatnms2 = unique(fin$cropcat2)
```
...
cropyearspan2 = fin %>%
  ...
  group_by(gen) %>%
  summarise(mn_dt=mean(dtha))
tm_compass() + tm_scale_bar() + tm_shape(cropyearspan1) + tm_sf('mn_dt', palette='RdYlBu', breaks = c(0,30,60,90,120,150,180), id='dt/ha', title='dt/ha')  + tm_shape(nrw) + tm_borders(lwd = 2, col='grey1')+ tm_layout(title='Fig.5 Average yield per district from 2005 to 2009', title.bg.color=TRUE)
tm_compass() + tm_scale_bar() + tm_shape(cropyearspan2) + tm_sf('mn_dt', palette='RdYlBu', breaks = c(0,30,60,90,120,150,180), id='dt/ha', title='dt/ha') + tm_layout(title='Fig.6 Average yield per district from 2010 to 2015', title.bg.color=TRUE) + tm_shape(nrw) + tm_borders(lwd = 2, col='grey1')
...
mn_cropkreis = fin %>%
  group_by(cropcat2,gen) %>%
  summarise(mn_dt = mean(dtha))
tm_shape(nrw) + 
  tm_borders(lwd = 2, col='grey1')+ 
  tm_compass() + tm_scale_bar() + 
tm_shape(mn_cropkreis) + 
  tm_sf('mn_dt', palette='RdYlBu', id='dt/ha', title='dt/ha') + 
...
#TODO Stadtkreise und Kreise mit geringer Landwirtschaft aussortieren
```
...
pestcrop = st_intersection(mn_kreis, pest) #insert crop index dataframe here
