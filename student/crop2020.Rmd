...
```{r crop index}
# TODO better visuals: colour of bubbles as gradient, alpha for whole bubbles available?, legend only for colour or size, legend title as index, divide values by 1000
#TODO maybe replace sum_dtha<10000 with NA so that polygons of the district are saved for intersection with pest-data
cropindex = fin %>%
  na.omit() %>%                     #sort out rows which don't fit to the 2nd crop classification
  filter(year>=2010) %>%            #sort out years before 2010
  group_by(cropcat2,gn) %>%         
  mutate(mn_dt = mean(dtha)) %>%    #average yield per crop category and disrtict
  ungroup(cropcat2) %>%
  group_by(gn) %>%
  mutate(sum_dtha = sum(mn_dt)) %>% #summarise the average yield of crop categories per district
  filter(sum_dtha<10000)            #sort out districts below 10000 dt/ha
tm_compass() + tm_scale_bar() + tm_shape(fin) + tm_polygons() + tm_shape(cropindex) + tm_bubbles(size='sum_dtha',  col='sum_dtha', style='pretty', scale=3)  + tm_shape(nrw) + tm_borders(lwd = 2, col='grey30')+ tm_layout(title='Fig.9 Crop index', title.bg.color=TRUE)
```
